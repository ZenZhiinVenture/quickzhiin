          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="glass">
              ...
              <DropdownMenuItem onClick={() => { setWizardType('SALES_QUOTE'); setIsWizardOpen(true); }}>
                <FileText className="mr-2 h-4 w-4" /> Edit (New)
              </DropdownMenuItem>
              ...
              <DropdownMenuItem
                ...
                onClick={() => setConfirmState({ isOpen: true, id: quote.id, type: 'quote', label: quote.number })}
              >
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="glass">
              ...
              <DropdownMenuItem onClick={async () => {
                try {
                  setActionLoading(true);
                  await tradeAPI.createDelivery({ orderId: order.id });
                  toast({ title: 'Success', description: 'Delivery created.' });
                  fetchData();
                } catch (e: any) {
                  toast({ variant: 'destructive', title: 'Error', description: e.response?.data?.message || 'Failed to create delivery' });
              ...
              <DropdownMenuItem onClick={async () => {
                try {
                  setActionLoading(true);
                  await tradeAPI.patchOrderStatus(order.id, 'COMPLETED');
                  toast({ title: 'Success', description: 'Order marked as completed.' });
                  fetchData();
                } catch (e: any) {
                  toast({ variant: 'destructive', title: 'Error', description: e.response?.data?.message || 'Failed to update status' });
                } finally { setActionLoading(false); }
              }}>
              ...
              <DropdownMenuItem
                className="text-red-500"
                onClick={() => setConfirmState({ isOpen: true, id: order.id, type: 'order', label: order.number })}
              >
                <Trash2 className="mr-2 h-4 w-4" /> Delete
              </DropdownMenuItem>
            </DropdownMenuContent>
      ...
      cell: ({ row }: any) => {
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="glass">
              <DropdownMenuLabel>Actions</DropdownMenuLabel>
              <DropdownMenuItem onClick={() => window.open(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000/api'}/invoice/${invoice.id}/pdf`, '_blank')}>
              ...
              <DropdownMenuItem onClick={async () => {
                try {
                  ...
                  toast.success('Email sent!', { id: 'email' });
                } catch { toast.error('Failed to send email', { id: 'email' }); }
              }}>
              ...
              <DropdownMenuItem onClick={async () => {
                try {
                  ...
                  toast.success('Submitted to LHDN!', { id: 'lhdn' });
                  fetchData();
                } catch (error: any) {
                  toast.error(error.response?.data?.message || 'Failed to submit', { id: 'lhdn' });
                }
              ...
              {invoice.lhdnStatus === 'Submitted' && (
                <DropdownMenuItem onClick={async () => {
                  try {
                    ...
                    fetchData();
                  } catch { toast.error('Failed to check status', { id: 'lhdn_status' }); }
                }}>
