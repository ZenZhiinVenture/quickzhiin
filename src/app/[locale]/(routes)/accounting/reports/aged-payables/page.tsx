...
export default function AgedPayablesReport() {
  ...
  const fetchReport = async () => {
    try {
      setLoading(true);
      const res = await reportAPI.getAgedPayables();
      ...
      toast({
        title: 'Error',
        description: err?.response?.data?.message || 'Failed to fetch Aged Payables report',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b pb-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold">{t('agedPayables') || 'Aged Payables'}</h1>
            <p className="text-muted-foreground">Monitor outstanding balances owed to suppliers</p>
          </div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <Card className="glass-card"><CardContent className="p-12 text-center text-muted-foreground">No outstanding payables found.</CardContent></Card>
      ) : (
          ...
          <div className="lg:col-span-3 space-y-6">
            <Card className="glass-card overflow-hidden shadow-2xl border-white/10">
              <CardHeader className="bg-muted/30 border-b border-white/5 p-8 text-center bg-gradient-to-r from-primary/5 to-transparent">
                <CardTitle className="text-3xl font-black tracking-tighter">AGED PAYABLES</CardTitle>
                    ...
                    <table className="w-full text-sm">
                        <thead className="bg-muted text-muted-foreground uppercase text-[10px] tracking-widest font-black">
                            <tr>
                                ...
                                <th className="px-6 py-4 text-right text-primary font-bold">Total Owed</th>
                            </tr>
                        </thead>
                            ...
                            {data.items.length === 0 ? (
                              <tr>
                                <td colSpan={7} className="px-8 py-12 text-center text-muted-foreground font-medium">
                                  Great job! You have no outstanding bills to pay.
                                </td>
                              </tr>
                            ) : (
                              data.items.map(item => (
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="p-6 rounded-3xl glass border border-white/10 flex flex-col gap-4 shadow-2xl">
                ...
                <p className="text-[10px] text-center text-muted-foreground font-medium uppercase tracking-tighter mt-4 opacity-50 px-2 leading-relaxed">
                    This report tracks unpaid bills to help you manage supplier payments and maintain good relationships.
                </p>
            </div>
          </div>
        </div>
